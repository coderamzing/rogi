...
export default function CalculatorSummary({
  ...
  const generateSummary = () => {
    ...
    if (calculatorType === "mortgage") {
      ...
      formattedData = `
        Current Balance: $${Number.parseFloat(
          currentBalance
        ).toLocaleString()} at ${currentRate}%
        New Rate: ${newRate}%
        Term: ${term} years
    ...
    if (calculatorType === "mortgage") {
      ...
      const totalInterest =
        monthlyPayment * 12 * amortizationPeriod - mortgageAmount;
      ...
      summaryText = `
        ...
        ${
          newLoanAmount > currentLoanAmount
            ? `You're increasing your mortgage by $${(
                newLoanAmount - currentLoanAmount
              ).toLocaleString()}. Make sure you have a good plan for using these additional funds.`
            : `You're reducing your mortgage by $${(
                currentLoanAmount - newLoanAmount
              ).toLocaleString()}, which is a positive step toward building equity.`
        }
      ...
      summaryText = `
        ...
        You're considering renewing your mortgage with a current balance of $${Number.parseFloat(
          currentBalance
        ).toLocaleString()} from a rate of ${currentRate}% to a new rate of ${newRate}% for a ${term}-year term.
        
        This renewal would save you $${
          monthlySavings?.toFixed(2) || "0.00"
        } per month, or $${
        totalSavings?.toFixed(2) || "0.00"
      } over the entire term.
        
        ${
          Number.parseFloat(newRate) < Number.parseFloat(currentRate)
            ? `The ${(
                Number.parseFloat(currentRate) - Number.parseFloat(newRate)
              ).toFixed(
                2
              )}% rate reduction represents a significant opportunity to save on interest.`
            : `The ${(
                Number.parseFloat(newRate) - Number.parseFloat(currentRate)
              ).toFixed(2)}% rate increase will impact your monthly budget.`
        }
      ...
      summaryText = `
        ...
        Based on your current mortgage balance of $${Number.parseFloat(
          balance
        ).toLocaleString()} at ${rate}% with a ${rateType} rate, your estimated penalty for breaking your mortgage would be approximately $${
        penalty?.toFixed(2) || "0.00"
      }.
            ...
            <div className="flex items-start space-x-3 mb-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500">
                  Personalized insights based on your calculation
                </p>
              </div>
            </div>

            <div className="whitespace-pre-line text-gray-700 text-sm leading-relaxed mb-4">
              {summary}
            </div>
